...
import { getMonthlySummaries, updateSummary } from '../services/logService';
import { useCompany } from '../context/CompanyContext';
import { CalendarDays, Sparkles, ChevronDown, ChevronUp, Edit2, Save, X } from 'lucide-react';
...
const MonthlyLogs = () => {
    ...
    const { data: summaries, isLoading } = useQuery({
        queryKey: ['monthlySummaries', selectedCompany?._id],
        queryFn: () => getMonthlySummaries(selectedCompany?._id),
    ...
    const updateMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries(['monthlySummaries']);
            setEditingId(null);
    ...
    const handleEditStart = (e, summary) => {
        ...
        // Ensure the month is expanded when editing
        setExpandedMonth(summary._id);
    };
    ...
    if (!summaries || summaries.length === 0) {
        return (
            <div className="text-center py-16">
                ...
                <h3 className="text-xl font-semibold text-text mb-2">No Monthly Summaries Yet</h3>
                <p className="text-muted">Monthly summaries are automatically generated on the last day of each month.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-4">
            {summaries.map((summary) => (
                ...
                >
                    <div
                        onClick={() => !editingId && setExpandedMonth(expandedMonth === summary._id ? null : summary._id)}
